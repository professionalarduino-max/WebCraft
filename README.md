...
**Light and darkness — torches finally mean something.**

- **A real light engine.** Before, the game had no lighting at all: a torch was only a bright texture, a cave looked exactly like the surface, and night was one flat dimming of everything on screen. Now every chunk carries two light maps (block light 0-15 and sky light 0-15) and the level is baked into the world's geometry:
  - **Dig down and it goes dark.** Sky light is poured down every column from the sky: it is 15 in the open, drops by one per block below the first solid block, so a mine gets darker as you descend, a roof casts a real shadow, and a hole you dig lets a shaft of daylight in.
  - **Torches light their room.** Light spreads outwards from every light source, one level per block, blocked by solid blocks and dimmed quickly by water (2 per block) and leaves (3 per block). A torch gives 14, so it lights a 13-block radius, a glowstone 15, magma 3 (a floor decoration, not a lamp) — exactly the Minecraft values.
  - **Light levels, not tint tricks:** 15 = full brightness, 0 = a barely visible silhouette instead of a black screen, so you can still cross a dark cave without seeing nothing at all.
  - **Daylight and torchlight are separate.** Faces lit by a torch are drawn by a different material that ignores the sun, so a lit room looks *exactly* the same at midnight as at noon — that is what makes a torch feel like a torch.
  - **Night is proper moonlight:** cool blue, dark but readable; the day got a little brighter to make the contrast honest. The Nether and the End have their own base glow instead of pitch black.
- **Every light source is covered** — torch, lantern, glowstone, sea lantern, shroomlight, jack o'lantern, end rod, redstone torch (7), redstone lamp when on, copper bulb when lit, lava (15) and portal blocks (11), plus armour-light from the spawn plaza lamps. Breaking a lamp or glowstone takes its light away immediately; covering a torch with a block does too.
- **Fast enough to never hitch:** the light queues are drained with a per-frame budget (a few hundred operations per placed block), a chunk's light is computed once when the chunk is generated, and a big incoming batch (your friend's city arriving, or the world snapshot when you join) is lit in one pass instead of per block — 552 edits in 64 ms. A full relight of a chunk shows up spread over a frame or two rather than freezing the game.
- **New:** `/light` shows the light level (block / sky) of the block you are looking at, if you ever want to check the numbers.
...
- **World safety (NEW)** — 200k-edit cap (10× bigger, `--max-deltas`) with a loud warning instead of a silent drop, compact v2 world files, gzipped `/export?gz=1` snapshots, `/rollback <minutes>` and `/restore` for operators, `/undo [seconds]` for players, and a `world` git-branch backup that the free Render host boots from
- **Anti-grief (NEW)** — `/claim [r]` territory (green border, server-enforced), `/unclaim`, `/claims`, `/lock` on chests/doors/furnaces (only the owner can open *or break* them), `/ban` `/unban` `/bans`
- **Co-op & quality of life (NEW)** — sleep-all-to-skip-night with 💤 in the Tab list, `/sethome` `/home` `/delhome` `/homes` (5 homes), `/tpa` `/tpaccept` `/tpdeny` `/back`, teams with friendly fire off (`/team`, `/tc`), and a `/stat` `/top` scoreboard of kills, deaths, blocks placed and broken
- **Lighting (NEW)** — a block-light and sky-light engine baked into the chunk geometry: torches, lanterns, glowstone, lamps, lava and portals light their surroundings, caves and mineshafts are dark, dug shafts let daylight in, water and leaves dim light, torch-lit faces stay bright at night (drawn unlit), night is blue moonlight, and `/light` prints the level of the block you look at
